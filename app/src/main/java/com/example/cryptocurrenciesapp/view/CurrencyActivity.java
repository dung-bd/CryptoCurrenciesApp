...
import com.example.cryptocurrenciesapp.utilities.Formatter;
import com.example.cryptocurrenciesapp.viewmodel.CurrencyViewModel;
import com.example.cryptocurrenciesapp.viewmodel.CurrencyViewModelFactory;
...
public class CurrencyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Currency currency1 = getIntent().getParcelableExtra(CURRENCY_INTENT_KEY);
        viewModel = new ViewModelProvider(this, new CurrencyViewModelFactory(currency1, new Database(this)))
    ...
    private void initChart(List<Detail> detail) {
        ...
        for (Detail el : detail) {
            entries.add(new Entry(el.getTime(), (float) el.getClose()));
        }
    ...
    private void initChipGroup() {
        ...
        chipGroup.setOnCheckedChangeListener((group, checkedId) -> {
            switch (checkedId) {
                case R.id.chip1h:
                    viewModel.fetchDetail("minute", 60);
                    break;
                case R.id.chip24h:
                    viewModel.fetchDetail("hourly", 24);
                    break;
                case R.id.chip7d:
                    viewModel.fetchDetail("hourly", 168);
                    break;
                case R.id.chip1m:
                    viewModel.fetchDetail("daily", 30);
                    break;
                case R.id.chip3m:
                    viewModel.fetchDetail("daily", 90);
                    break;
                case R.id.chip1y:
                    viewModel.fetchDetail("daily", 365);
                    break;
                case R.id.chipMax:
                    viewModel.fetchDetail("daily", 2000);
                    break;
            }
        });
    }
